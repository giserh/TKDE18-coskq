 ...
 *  return an coskq_config_t pointer pointing to an object that storing the config
 */
coskq_config_t* read_config_coskq( )
{
	...
	//data.
	fscanf( c_fp, "%i%i%s", &cfg->dim, &cfg->obj_n, cfg->loc_file);
 ...
 * note that k_node_v is the pointer pointing the last element of the list
 */
...
void print_coskq_stat( coskq_config_t* cfg, int cnt)
{
	...
	if( !( s_fp = fopen( COSKQ_STAT_FILE, "w")))
	{
 ...
 */
data_t*	read_data_coskq( coskq_config_t* cfg, std::unordered_map<KEY_TYPE, KEY_TYPE>* keyfreq_hashmap)
{
	int i, j;
	KEY_TYPE key;
	char des;
   ...
   // std::unordered_map<KEY_TYPE, KEY_TYPE> keywordfreq_hashmap = *keyfreq_hashmap;
	...
	for( i=0; i<data_v->obj_n; i++)
	{
		...
		while( ( tok = strtok( NULL, " ,")))
		{
			key = atoi(tok);
			add_keyword_entry( k_node_v, key);
            //-- preprocess to get keyword freq
            //-- store in hash map
          //  printf("key:%f\n",key);
            std::unordered_map<KEY_TYPE,KEY_TYPE>::iterator got = keyfreq_hashmap->find (key);
            if ( got == keyfreq_hashmap->end() )//not found
                (*keyfreq_hashmap)[key]=1;
            else
                (*keyfreq_hashmap)[key]++;
            //printf("key:%f\n",key);
            //--
