		...
		while( obj_set_list_v1->next != NULL)
		{
			...
			while( obj_set_list_v2 != NULL)
			{
				...
				if( ( obj_v = join_check_obj_set( obj_set_v1, obj_set_v2)))
				{
					...
					//
					if( comp_cost( cost_tag, obj_set_v, q) < cost &&
...
obj_set_t* exhaustive_node_set_search( cns_t* cns_v, B_KEY_TYPE cost, query_t* q)
{
	...
	while( obj_set_list_iter2 != NULL)
	{
		...
		cost_tmp = comp_cost( cost_tag, obj_set_list_iter2->obj_set_v, q);
 ...
 */
obj_set_t* Cao_Exact( query_t* q,rusage query_sta,int& cntTimeOut)
{
#ifndef WIN32
    struct rusage cur;
    float sys_t, usr_t;
...
#ifndef WIN32
        GetCurTime( &cur);
        GetTime( &query_sta, &cur, &usr_t, &sys_t);
        if(usr_t>3600.0 ){
            
        //    printf("usr_t:%f\tsys_t:%f\t\n",usr_t,sys_t);
            release_cns( cns_v);
            bst_delete( heap_v, top);
            free( top);			//Memory usage consideration.
            cntTimeOut++;
            break;//V will not be null, ok to return
        }
#endif
        /*t*/
